...
static struct cache_inode *alloc_cache_inode(struct sfs_fs *sfs, ino_t real,
					     uint32_t ino, uint16_t type)
{
...
init_dir_cache_inode(struct cache_inode *current, struct cache_inode *parent)
{
	...
	assert(inode->type == SFS_TYPE_DIR
	       && parent->inode.type == SFS_TYPE_DIR);
	assert(inode->nlinks == 0 && inode->dirinfo.slots == 0
	       && inode->dirinfo.parent == 0);
	inode->nlinks++, parent->inode.nlinks++, inode->dirinfo.parent =
	    parent->ino;
}
...
struct sfs_fs *create_sfs(int imgfd)
{
	...
	if ((next_ino =
	     SFS_BLKN_FREEMAP + (ninos + SFS_BLKBITS - 1) / SFS_BLKBITS) >=
	    ninos) {
...
write_block(struct sfs_fs *sfs, void *data, size_t len, uint32_t ino)
{
	...
	ssize_t ret;
	if ((ret =
	     pwrite(sfs->imgfd, data, SFS_BLKSIZE, offset)) != SFS_BLKSIZE) {
...
{
	const char *expect = ".img", *ext =
	    imgname + strlen(imgname) - strlen(expect);
...
void open_dir(struct sfs_fs *sfs, struct cache_inode *current);
void open_file(struct sfs_fs *sfs, struct cache_inode *file,
	       const char *filename, int fd);
void open_link(struct sfs_fs *sfs, struct cache_inode *file,
	       const char *filename);
...
static void
__append_block(struct sfs_fs *sfs, struct cache_inode *file, uint32_t ino,
	       const char *filename)
{
...
static void
append_block_size(struct sfs_fs *sfs, struct cache_inode *file, size_t size,
		  uint32_t ino, const char *filename)
{
	struct inode *inode = &(file->inode);
...
static void
append_block_slot(struct sfs_fs *sfs, struct cache_inode *file, uint32_t ino,
		  const char *filename)
{
	struct inode *inode = &(file->inode);
	assert(inode->type == SFS_TYPE_DIR);
...
static void
add_entry(struct sfs_fs *sfs, struct cache_inode *current,
	  struct cache_inode *file, const char *name)
{
	static struct sfs_entry __entry, *entry = &__entry;
	assert(current->inode.type == SFS_TYPE_DIR
...
static void
add_dir(struct sfs_fs *sfs, struct cache_inode *parent, const char *dirname,
	int curfd, int fd, ino_t real)
{
	assert(search_cache_inode(sfs, real) == NULL);
	struct cache_inode *current =
	    alloc_cache_inode(sfs, real, 0, SFS_TYPE_DIR);
	init_dir_cache_inode(current, parent);
	safe_fchdir(fd), subpath_push(sfs, dirname);
	open_dir(sfs, current);
	safe_fchdir(curfd), subpath_pop(sfs);
	add_entry(sfs, parent, current, dirname);
}

static void
add_file(struct sfs_fs *sfs, struct cache_inode *current, const char *filename,
	 int fd, ino_t real)
{
...
static void
add_link(struct sfs_fs *sfs, struct cache_inode *current, const char *filename,
	 ino_t real)
{
	struct cache_inode *file =
	    alloc_cache_inode(sfs, real, 0, SFS_TYPE_LINK);
	...
	while ((direntp = readdir(dir)) != NULL) {
		...
		if (S_ISLNK(stat->st_mode)) {
			...
			if (S_ISDIR(stat->st_mode)) {
				add_dir(sfs, current, name, dirfd(dir), fd,
					stat->st_ino);
...
void
open_file(struct sfs_fs *sfs, struct cache_inode *file, const char *filename,
	  int fd)
{
	static char buffer[SFS_BLKSIZE];
